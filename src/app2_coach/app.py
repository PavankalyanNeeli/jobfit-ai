    ...
    if not job_desc or not resume_text:
        ...
        with st.spinner("🧠 AI is simulating thousands of resume variations..."):
            ...
            if st.button("Generate Roadmap", type="secondary"):
                ...
                st.markdown("<br><h4 style='color:#58a6ff;'>Your Elite Study Schedule</h4>", unsafe_allow_html=True)
                
                # Build Markdown string for download
                download_str = f"# JobFit AI - Elite Learning Roadmap\\n\\n"
                download_str += f"**Goal:** Job Ready\\n**Pace:** {hours_per_day} hours/day\\n**Budget:** {budget}\\n\\n---\\n\\n"
                
                for step in roadmap:
                    # Elite UI Card for each step
                    st.markdown(f"""
                    <div style="background: rgba(35, 41, 54, 0.4); border: 1px solid #30363d; border-radius: 10px; padding: 20px; margin-bottom: 15px; position: relative; overflow: hidden;">
                        <div style="position: absolute; top: 0; left: 0; width: 6px; height: 100%; background: linear-gradient(180deg, #bb86fc, #58a6ff);"></div>
                        <h4 style="margin-bottom: 5px; color: #ffffff; margin-top: 0;">📅 Day {step['start_day']} - {step['end_day']} : Master {step['skill'].upper()}</h4>
                        <div style="display: flex; gap: 20px; font-size: 0.95rem; margin-bottom: 10px;">
                            <span style="color: #8b949e;">⏳ <b>Total Time:</b> {step['total_hours']} hours</span>
                            <span style="color: #8b949e;">🎯 <b>Pace:</b> {hours_per_day} hrs/day</span>
                        </div>
                        <div style="background: rgba(0,0,0,0.3); padding: 10px; border-radius: 6px; color: #c9d1d9;">
                            📚 <b>Recommended Resource:</b> <i>{step['resource']}</i>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # Add to download string
                    download_str += f"### Day {step['start_day']} - {step['end_day']}: Master {step['skill'].upper()}\\n"
                    download_str += f"- **Total Time:** {step['total_hours']} hours\\n"
                    download_str += f"- **Resource:** {step['resource']}\\n\\n"
                ...
                # Download Button
                download_str += f"---\\n*Generated by JobFit AI*\\n*Estimated Completion: {total_days} days*"
                st.download_button(
                    label="💾 Download Roadmap (Markdown)",
                    data=download_str,
                    file_name="JobFit_AI_Roadmap.md",
                    mime="text/markdown",
                    type="primary"
                )
